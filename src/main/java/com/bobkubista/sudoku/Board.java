...
public class Board {
	...
	public boolean isValueAlreadyUsed(int row, int column, Integer solution) {
		return isInColumn(column, solution) || isInRow(row, solution) || isInBlock(row, column, solution);
	}
	...
	public boolean isSolved() {
		return Stream.of(cells).allMatch(row -> Stream.of(row).allMatch(Cell::isSolved));
	}
	...
	private String cellsArrayToString() {
		return Stream.of(cells)
				.map(row -> Stream.of(row).map(this::getStringValueForCell).collect(Collectors.joining(",")))
